...
export const TimesheetView: React.FC = () => {
  ...
  const { timeEntries, clockIn, clockOut, startBreak, endBreak } =
    useTimeEntries();
  ...
  const employeesWithTimeData = useMemo(() => {
    return filteredEmployees.map((employee) => {
      const todayEntry = timeEntries.find(
        (entry) =>
          entry.employeeId === employee._id && entry.date === selectedDate
      );
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-green-600">
                  {
                    employeesWithTimeData.filter(
                      (emp) => emp.timeEntry?.status === "checked-in"
                    ).length
                  }
                </p>
              </div>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-orange-600">
                  {
                    employeesWithTimeData.filter(
                      (emp) => emp.timeEntry?.status === "on-break"
                    ).length
                  }
                </p>
              </div>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-gray-600">
                  {
                    employeesWithTimeData.filter(
                      (emp) => emp.timeEntry?.status === "checked-out"
                    ).length
                  }
                </p>
              </div>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-purple-600">
                  {employeesWithTimeData
                    .reduce(
                      (sum, emp) => sum + (emp.timeEntry?.extraHours || 0),
                      0
                    )
                    .toFixed(1)}
                  h
                ...
                {employeesWithTimeData.map((employee) => (
                  ...
                  >
                    ...
                    <td className="py-4 px-4 text-center font-mono">
                      ...
                      employee.timeEntry?.breakEnd
                        ? `${formatTime(
                            employee.timeEntry.breakStart
                          )} - ${formatTime(employee.timeEntry.breakEnd)}`
                        : employee.timeEntry?.breakStart
                    ...
                    <td className="py-4 px-4 text-center">
                      <div>
                        <p className="font-medium">
                          {employee.timeEntry?.totalHours
                            ? `${employee.timeEntry.totalHours}h`
                            : "--"}
                        </p>
                      ...
                      <div className="flex justify-center space-x-2">
                        {employee.canClockIn && (
                          <Button
                            ...
                            variant="success"
                            onClick={() =>
                              handleAction("clockIn", employee._id)
                            }
                          >
                        ...
                        {employee.canStartBreak && (
                          <Button
                            ...
                            variant="outline"
                            onClick={() =>
                              handleAction("startBreak", employee._id)
                            }
                          >
                        ...
                        {employee.canEndBreak && (
                          <Button
                            ...
                            variant="secondary"
                            onClick={() =>
                              handleAction("endBreak", employee._id)
                            }
                          >
                            <Play className="w-4 h-4 mr-1" />
                            Voltar
                        ...
                        {employee.canClockOut && (
                          <Button
                            ...
                            variant="danger"
                            onClick={() =>
                              handleAction("clockOut", employee._id)
                            }
                          >
